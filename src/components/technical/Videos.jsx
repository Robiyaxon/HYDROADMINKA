...
export const Videos = () => {
    ...
    let images = null;
    images = useSelector(state => state.technicalPage ? state.technicalPage : null);
    ...
    const onSubmit = (data) => {
        !imageId ? dispatch(getTechnicalVideoCreate({ selectedImage, videoUrlYoutube: data.videoUrlYoutube })) :
            dispatch(getTechnicalVideoUpdate({ selectedImage, videoUrlYoutube: data.videoUrlYoutube, id: imageId.id, originalPath: imageId.videoUrlYoutube, selectedI }));
        setImageId(null)
        setSelectedI(false);
        setModalOpen(false);
    }
        ...
        <div>
            <Modal isOpen={modalOpen} toggle={toggle} >
                ...
                <ModalBody>
                    <Form
                        onSubmit={onSubmit}
                        initialValues={imageId && { videoUrlYoutube: imageId.videoUrlYoutube }}
                        validate={values => {
                            const errors = {}
                            if (!values.videoUrlYoutube) {
                                if (!values.videoUrlYoutube) { errors.videoUrlYoutube = 'Invalid video Url Youtube address' }
                            }
                            return errors
                        }}
                                    ...
                                    <Field name="videoUrlYoutube">
                                        {({ input, meta }) => (
                                            <div>
                                                ...
                                                <Input type='text' {...input} placeholder='Video Url Youtube' />
